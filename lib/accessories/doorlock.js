...
const {
  ...
  doorLockConfigurationOperation,
...
class DoorLock extends VerisureAccessory {
  ...
  getAutoLockState(callback) {
    ...
    this.installation.client(doorLockConfigurationOperation(this.serialNumber))
      .then(({ installation: { smartLock: { configuration } } }) => {
        this.autoLockState = DoorLock.resolveAutoLockState(configuration);
  ...
  getAudioState(callback) {
    ...
    this.installation.client({ url: this.configUrl })
      .then(({ installation: { smartLock: { configuration } } }) => {
        this.audioState = configuration.volume !== this.platformConfig.audioOffValue;
