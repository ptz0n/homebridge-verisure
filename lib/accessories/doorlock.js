...
class DoorLock extends VerisureAccessory {
  constructor(...args) {
    ...
    this.lockName = VerisureAccessory.getUniqueAccessoryName(`SmartLock (${this.config.area})`);
  ...
  getDoorLockState() {
    ...
    return this.installation.client(request)
      ...
      .then((doorLock) => {
        if (!doorLock) {
          throw Error(`Could not find lock state for ${this.lockName}.`);
        }
        return doorLock;
      });
  }
  ...
  getAutoLockState(callback) {
    ...
    this.installation.client({ uri: this.configUrl })
      .then((config) => {
  ...
  getServices() {
    ...
    this.lockService = new Service.LockMechanism(this.lockName);
