...
class DoorLock extends VerisureAccessory {
  ...
  getAudioState(callback) {
    this.installation.client({ url: this.configUrl })
      .then((config) => {
        this.audio = config.volume !== this.platformConfig.audioOffValue;
        callback(null, this.audio);
      })
      .catch(callback);
  }
  ...
  setAudioState(value, callback) {
    ...
    const mode = value ? this.platformConfig.audioOnValue : this.platformConfig.audioOffValue;
