...
class DoorLock extends VerisureAccessory {
  ...
  static resolveAudioLevel(config) {
    return config.volume;
  }

  getAudioLevelState(callback) {
    this.log('Getting current audio level config.');

    this.installation.client({ url: this.configUrl })
      .then((config) => {
        this.audioLevelState = DoorLock.resolveAudioLevel(config);
        callback(null, this.audioLevelState);
      })
      .catch(callback);
  }

  setAudioLevelState(value, callback) {
    this.log(`Setting audio level to: ${value}`);

    this.audioLevelState = value;
