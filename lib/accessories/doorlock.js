...
class DoorLock extends VerisureAccessory {
  constructor(...args) {
    ...
    this.speakerName = VerisureAccessory.getUniqueAccessoryName(`Volume (${this.config.area})`);
  ...
  static resolveMutedState(config) {
    return config.volume === 'SILENCE';
  }

  getMuteState(callback) {
    this.installation.client({ url: this.configUrl })
      .then((config) => {
        this.muted = DoorLock.resolveMutedState(config);
  ...
  setMuteState(value, callback) {
    ...
    const volume = value ? 'SILENCE' : 'HIGH';
